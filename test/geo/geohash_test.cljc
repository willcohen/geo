(ns geo.geohash-test
  (:require [geo.crs :as crs]
            ...
            [geo.io :as gio]
            #?(:clj [geo.geohash :as gh]
               :cljs [geo.geohash :as gh :refer [Geohash]])
            [clojure.test :refer [deftest testing is]])
  #?(:clj (:import (ch.hsr.geohash GeoHash)
...
(deftest gh
  (testing "geohash"
      (is (partial instance? #?(:clj GeoHash
                                :cljs Geohash))
          (gh/geohash 50 20 64)))

  (testing "from string"
    (is (#?(:clj spatial/geohash-point
            :cljs spatial/jsts-point) 57.64911063015461 10.407439693808556)
        (gh/geohash-center (gh/geohash "u4pruydqqvj"))))

  (testing "interchangeable center"
    (is true
        (jts/same-geom? (#?(:clj spatial/jts-point
                            :cljs spatial/jsts-point)
                         (gh/geohash-center (gh/geohash "u4p4uydqqvj")))
                        (#?(:clj spatial/jts-point
                            :cljs spatial/jsts-point)
                         (spatial/center (gh/geohash "u4p4uydqqvj"))))))

  (testing "subdivide"
    (is [0 (dec (- (long (Math/pow 2 63))))]
        (map #(.longValue ^GeoHash %) (gh/subdivide (gh/geohash ""))))

    (is ["scu20" "scu21" "scu22" "scu23" "scu24" "scu25" "scu26"
        ...
        (map gh/string (gh/subdivide (gh/geohash "scu2") 25))))

  (testing "subdivide for level range"
    (let [subdivided (gh/subdivide-levels (gh/geohash "0") 5 8)
          level-checker (fn [level]
                          #(= level (gh/significant-bits %)))]
      (is (= 15 (count subdivided)))
      ;; (range 2^k) where k = (level - min_level) for each level in range
      (is (= (mapcat range [1 2 4 8])
             (map #?(:clj #(.ord ^GeoHash %)
                     :cljs identity)
                  subdivided)))))
...
(deftest rings
  ...
  ; infinite grid.
  [gh/northern-neighbor (fn [[lat long]] [(inc lat) long])
   gh/southern-neighbor (fn [[lat long]] [(dec lat) long])
   gh/eastern-neighbor  (fn [[lat long]] [lat (inc long)])
   gh/western-neighbor  (fn [[lat long]] [lat (dec long)])]

    (testing "Check mock"
      (is (= (gh/northern-neighbor [1 2])
             [2 2])))

    (testing  "Square ring"
      (is (thrown? #?(:clj AssertionError
                      :cljs js/Error)
                   (gh/square-ring [0 0] -1)))
      (is (thrown? #?(:clj AssertionError
                      :cljs js/Error)
                   (gh/square-ring [0 0] 0)))
      (is (= [[0 0]]
             (gh/square-ring [0 0] 1)))
      (is (thrown? #?(:clj AssertionError
                      :cljs js/Error)
                   (gh/square-ring :foo 2)))
      (is (= [[ 1  1] [ 0  1]   ; south
              [-1  1] [-1  0]   ; west
              [-1 -1] [0  -1]   ; north
              [ 1 -1] [ 1  0]]) ; east
          (gh/square-ring [0 0] 3))
      (is (thrown? #?(:clj AssertionError
                      :cljs js/Error)
                   (gh/square-ring [1 1] 4)))
      (is (= [[ 2  2] [ 1  2] [ 0  2] [-1  2]    ; south
              [ -2 2] [-2  1] [-2  0] [-2 -1]    ; west
              [-2 -2] [-1 -2] [ 0 -2] [ 1 -2]    ; north
              [ 2 -2] [ 2 -1] [ 2  0] [ 2  1]]   ; east
             (gh/square-ring [1 1] 5))))

    (testing "Concentric square rings"
      (is (= [[[3 -4]]]
             (take 1 (gh/concentric-square-rings [3 -4]))))
      (is (= [[[0 0]]
              (gh/square-ring [0 0] 3)]
             (take 2 (gh/concentric-square-rings [0 0])))
          (is (= (cons [[-2 5]]
                       (map (partial apply gh/square-ring) [[[-2 5] 3]
                                                            [[-1 6] 5]
                                                            [[0 7] 7]]))
                 (take 4 (gh/concentric-square-rings [-2 5]))))))))
